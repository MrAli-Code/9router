...
# **ناین‌روتر (9Router) \- مسیریاب هوش مصنوعی و بهینه‌ساز رایگان توکن**

**هرگز از کدزدن دست نکشید. با کمک RTK بین ۲۰ تا ۴۰ درصد در مصرف توکن‌هایتان صرفه‌جویی کنید \+ سوییچ خودکار (Fallback) به مدل‌های رایگان و ارزان هوش مصنوعی.**  
**تمام ابزارهای کدنویسی هوش مصنوعی خود (مثل Claude Code, Cursor, Antigravity, Copilot, Codex, Gemini, OpenCode, Cline, OpenClaw...) را به بیش از ۴۰ ارائه‌دهنده و ۱۰۰+ مدل مختلف متصل کنید.**  
[🚀 شروع سریع](https://www.google.com/search?q=%23-%D8%B4%D8%B1%D9%88%D8%B9-%D8%B3%D8%B1%DB%8C%D8%B9&authuser=1) • [💡 ویژگی‌های کلیدی](https://www.google.com/search?q=%23-%D9%88%DB%8C%DA%98%DA%AF%DB%8C%D9%87%D8%A7%DB%8C-%DA%A9%D9%84%DB%8C%D8%AF%DB%8C&authuser=1) • [📖 راهنمای نصب](https://www.google.com/search?q=%23-%D8%B1%D8%A7%D9%87%D9%86%D9%85%D8%A7%DB%8C-%D9%86%D8%B5%D8%A8&authuser=1) • [🌐 وب‌سایت](https://9router.com)  
[🇻🇳 Tiếng Việt](https://www.google.com/search?q=./i18n/README.vi.md&authuser=1) • [🇨🇳 中文](https://www.google.com/search?q=./i18n/README.zh-CN.md&authuser=1) • [🇯🇵 日本語](https://www.google.com/search?q=./i18n/README.ja-JP.md&authuser=1) • [🇷🇺 Русский](https://www.google.com/search?q=./i18n/README.ru.md&authuser=1)

## **🤔 چرا 9Router؟**

**هدر دادن پول، سوزاندن توکن‌ها و برخورد با محدودیت‌ها را متوقف کنید:**

* ❌ سهمیه اشتراک ماهانه‌تان بدون اینکه کامل استفاده شود منقضی می‌شود.  
* ❌ محدودیت‌های درخواست (Rate limits) شما را در اوج کدنویسی متوقف می‌کنند.  
* ❌ خروجی ابزارها (مثل git diff، grep، ls و...) توکن‌های شما را به سرعت می‌سوزانند.  
* ❌ هزینه‌های بالای API (بین ۲۰ تا ۵۰ دلار در ماه برای هر ارائه‌دهنده).  
* ❌ جابجایی کلافه‌کننده و دستی بین ارائه‌دهندگان مختلف.

**ناین‌روتر (9Router) این مشکلات را حل می‌کند:**

* ✅ **بهینه‌ساز توکن RTK:** فشرده‌سازی خودکار محتوای tool\_result که در هر درخواست ۲۰ تا ۴۰ درصد در مصرف توکن صرفه‌جویی می‌کند.  
* ✅ **حداکثر استفاده از اشتراک‌ها:** ردیابی سهمیه مصرفی تا قبل از تمدید مجدد، از تمام حجم خود استفاده کنید.  
* ✅ **سوییچ خودکار (Auto Fallback):** انتقال نرم و بدون توقف از مدل‌های اشتراکی → ارزان → رایگان.  
* ✅ **پشتیبانی از چند اکانت:** توزیع بار چرخشی (Round-robin) بین اکانت‌های مختلفِ یک ارائه‌دهنده.  
* ✅ **همه‌کاره:** سازگار با Claude Code, Codex, Cursor, Cline و هر ابزار CLI دیگر.
...
│ • RTK Token Saver (کاهش توکن خروجی ابزارها) │  
│ • تبدیل فرمت (OpenAI ↔ Claude)              │  
│ • ردیابی میزان سهمیه (Quota tracking)       │  
│ • رفرش خودکار توکن‌ها                       │  
└──────┬──────────────────────────────────────┘  
       │  
       ├─→ \[سطح ۱: اشتراکی\] Claude Code, Codex, GitHub Copilot  
       │   ↓ اتمام سهمیه  
       ├─→ \[سطح ۲: ارزان\] GLM ($0.6/1M), MiniMax ($0.2/1M)  
       │   ↓ محدودیت بودجه  
       └─→ \[سطح ۳: رایگان\] Kiro, OpenCode Free, Vertex ($300 credits)

نتیجه: هیچ‌وقت کدنویسی‌تان متوقف نمی‌شود، هزینه‌ها به حداقل می‌رسد \+ ۲۰ تا ۴۰ درصد صرفه‌جویی توکن با RTK

## **⚡ شروع سریع**

**۱. نصب به صورت سراسری (Global):**

Bash  
npm install \-g 9router  
9router

🎉 داشبورد شما در آدرس http://localhost:20128 باز می‌شود.  
**۲. اتصال یک ارائه‌دهنده رایگان (بدون نیاز به ثبت‌نام):**  
در داشبورد به بخش Providers بروید → به **Kiro AI** (مدل Claude نامحدود و رایگان) یا **OpenCode Free** (بدون نیاز به احراز هویت) متصل شوید → تمام\!  
**۳. استفاده در ابزار CLI شما:**

Plaintext  
تنظیمات در Claude Code / Codex / OpenClaw / Cursor / Cline:  
  Endpoint: http://localhost:20128/v1  
  API Key: \[از داشبورد کپی کنید\]  
  Model: kr/claude-sonnet-4.5

**به همین سادگی\!** حالا می‌توانید با مدل‌های رایگان هوش مصنوعی کد بزنید.  
**روش جایگزین: اجرا مستقیم از روی سورس کد (همین ریپازیتوری):**  
از آنجا که پکیج این ریپازیتوری خصوصی است (9router-app)، اجرای محلی (Local) از طریق سورس یا داکر بهترین روش توسعه است.
...
حالت پروداکشن (عملیاتی):

Bash  
npm run build  
PORT=20128 HOSTNAME=0.0.0.0 NEXT\_PUBLIC\_BASE\_URL=http://localhost:20128 npm run start

آدرس‌های پیش‌فرض:

* داشبورد: http://localhost:20128/dashboard  
* ای‌پی‌آی سازگار با OpenAI: آدرس http://localhost:20128/v1

## **🛠️ ابزارهای CLI پشتیبانی‌شده**

ناین‌روتر به صورت یکپارچه با تمام ابزارهای مهم کدنویسی هوش مصنوعی کار می‌کند:  
*(لیست ابزارها اعم از Cursor، Claude-Code، Cline، Copilot و ...)*

## **🌐 ارائه‌دهندگان پشتیبانی‌شده**

### **🔐 ارائه‌دهندگان مبتنی بر OAuth**

شامل Claude-Code, Codex, Cursor, GitHub و...

### **🆓 ارائه‌دهندگان کاملاً رایگان**

* **Kiro AI:** شامل مدل‌های Claude 4.5 \+ GLM-5 \+ MiniMax (نامحدود و رایگان)  
* **OpenCode Free:** بدون نیاز به احراز هویت با قابلیت دریافت خودکار مدل‌ها (نامحدود و رایگان)  
* **Vertex AI:** شامل مدل‌های Gemini 3 Pro \+ GLM-5 \+ DeepSeek (با ۳۰۰ دلار اعتبار رایگان اولیه)

**توجه:** پلن‌های رایگان iFlow، Qwen و Gemini CLI در سال ۲۰۲۶ متوقف شدند. لطفاً به جای آن‌ها از Kiro / OpenCode Free / Vertex استفاده کنید.

### **🔑 ارائه‌دهندگان مبتنی بر کلید API (بیش از ۴۰ مورد)**

پشتیبانی از OpenAI, Anthropic, Gemini, DeepSeek, OpenRouter, Groq و ده‌ها ارائه‌دهنده دیگر.

## **💡 ویژگی‌های کلیدی**

| ویژگی | عملکرد آن چیست؟ | چرا اهمیت دارد؟ |
| ----: | ----: | ----: |
| 🚀 **ذخیره‌ساز توکن RTK** | فشرده‌سازی خروجی ابزارها (git diff, grep, ls...) قبل از ارسال به LLM | صرفه‌جویی **۲۰ تا ۴۰ درصدی توکن‌های ورودی** در هر درخواست |
| 🧠 **ذخیره‌ساز Headroom** | یک پروکسی اختیاری برای فشرده‌سازی در مسیر /v1/compress | ذخیره توکن‌های Context بدون نیاز به تغییر کلاینت‌ها |
| 🪨 **حالت غارنشین (Caveman)** | تزریق پرامپتی خاص تا هوش مصنوعی پاسخ‌های کوتاه، فنی و بدون اضافه‌گویی بدهد | صرفه‌جویی **تا ۶۵ درصد در توکن‌های خروجی** |
| 🐴 **مدل موی دم‌اسبی (Ponytail)** | پرامپت "توسعه‌دهنده ارشد تنبل" → کدهای مینیمال، فقط در حد نیاز و بدون پیچیدگی اضافه | **کاهش توکن خروجی و کاهش نیاز به ریفکتور کد** |
| 🎯 **فال‌بک هوشمند ۳ سطحی** | مسیریابی خودکار: اشتراکی → ارزان → رایگان | کدنویسی بدون توقف و با آپتایم ۱۰۰٪ |
| 📊 **ردیابی زنده سهمیه‌ها** | شمارش زنده توکن‌ها \+ تایمر ریست شدن سهمیه | استفاده حداکثری از پولی که برای اشتراک داده‌اید |
| 🔄 **تبدیل فرمت** | تبدیل ساختار درخواست‌ها بین OpenAI ↔ Claude ↔ Gemini و... | سازگاری 9Router با هر ابزار CLI موجود در بازار |

*(در این بخش توضیحات عمیق‌تری درباره نحوه کار RTK، Headroom و استراتژی توسعه‌دهنده تنبل (Ponytail) داده شده است که باعث می‌شود هوش مصنوعی از کدهای اضافه (Boilerplate) دوری کرده و یک‌راست سراغ اصل مطلب برود).*

## **💰 قیمت‌گذاری در یک نگاه**

*(در اینجا جدولی از هزینه‌های ارائه‌دهندگان مختلف آمده است. فراموش نکنید که **ناین‌روتر کاملاً رایگان است** و هزینه‌ها صرفاً مربوط به ارائه‌دهندگانی است که خودتان انتخاب می‌کنید).*  
**💡 یک ترفند حرفه‌ای:** ترکیب RTK \+ Kiro AI \+ OpenCode Free \= **هزینه صفر مطلق \+ ۲۰ تا ۴۰ درصد صرفه‌جویی در مصرف توکن\!**

### **📊 درک هزینه‌ها و صورت‌حساب‌ها در داشبورد 9Router**

**واقعیت ماجرا:**  
✅ **نرم‌افزار 9Router کاملاً رایگان است و همیشه رایگان می‌ماند.**  
✅ **هزینه‌ای که در داشبورد می‌بینید صرفاً نمایشی و برای پیگیری است** (نه یک صورت‌حساب واقعی).  
✅ **شما فقط به خود ارائه‌دهندگان پول می‌دهید** (آن هم در صورت استفاده از سرویس‌های پولی).  
❌ **ناین‌روتر هرگز برای شما فاکتور صادر نمی‌کند یا از کارتتان پولی کسر نمی‌کند.**  
**نمایش هزینه در داشبورد به چه معناست؟**  
اگر داشبورد نشان می‌دهد که شما ۲۹۰ دلار هزینه کرده‌اید، اما در حال استفاده از یک ارائه‌دهنده رایگان (مثل Kiro) هستید، این یعنی شما با استفاده از 9Router **مبلغ ۲۹۰ دلار صرفه‌جویی کرده‌اید\!**

## **🎯 سناریوهای کاربردی**

### **مورد ۱: "من اشتراک Claude Pro دارم"**

**مشکل:** سهمیه‌ام هدر می‌رود و وقتی سنگین کد می‌زنم به لیمیت می‌خورم.  
**راه‌حل 9Router:** ابتدا از تمام ظرفیت اشتراکتان استفاده کنید، سپس در صورت اتمام سهمیه، به صورت خودکار به مدل ارزان GLM متصل شوید و به عنوان آخرین سنگر از مدل رایگان Kiro استفاده کنید.

### **مورد ۲: "می‌خواهم هیچ هزینه‌ای نکنم (صفر مطلق)"**

**مشکل:** بودجه‌ای برای خرید اشتراک ندارم اما یک هوش مصنوعی قابل اعتماد برای کدنویسی می‌خواهم.  
**راه‌حل 9Router:** ساخت یک «کمبو (Combo)» کاملاً رایگان شامل: Kiro (Claude 4.5 رایگان) \+ Vertex. نتیجه: کیفیت در سطح پروداکشن، بدون یک ریال هزینه.

## **❓ سوالات متداول (FAQ)**

## **📖 راهنمای راه‌اندازی (Setup)**

*(بخش راه‌اندازی شامل اتصال به کلود، داکر، تنظیمات متغیرهای محیطی .env و معرفی API در این قسمت مستندات اصلی گنجانده شده است).*

### **🚀 استقرار (Deployment)**

می‌توانید 9Router را روی لپ‌تاپ خود، سرورهای مجازی (VPS) یا از طریق داکر (Docker) اجرا کنید.  
**نمونه دستور داکر (شروع سریع):**
...
docker run \-d \\  
  ...
  \-p 20128:20128 \\  
  \-v "$HOME/.9router:/app/data" \\  
  \-e DATA\_DIR=/app/data \\  
  decolua/9router:latest

سپس مرورگر خود را باز کرده و به http://localhost:20128 بروید.

## **📄 لایسنس**

تحت لیسانس MIT \- برای جزئیات بیشتر فایل [LICENSE](https://www.google.com/search?q=LICENSE&authuser=1) را ببینید.
